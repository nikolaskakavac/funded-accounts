...
const CryptoPaymentPage = ({ navigate, token, planId }) => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-black via-emerald-950 to-black text-slate-50">
        <p className="font-sans text-[15px] text-emerald-100/90">{t('crypto.loadingShort', lang)}</p>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-black via-emerald-950 to-black text-slate-50">
        <p className="font-sans text-[15px] text-emerald-100/90">{t('crypto.noData', lang)}</p>
      </div>
    );
  }
