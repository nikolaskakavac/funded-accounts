...
import { getMe, requestCashOut } from '../api';
...
const Dashboard = ({ navigate, token, onLogout }) => {
  ...
  const handleCashOut = async () => {
    ...
    try {
      ...
      if (res.success) {
        setCashOutMessage('Javicemo vam se u narednih 48h.');
        setTimeout(() => setCashOutMessage(''), 3000);
      } else {
        setCashOutMessage(res.message || 'Greška pri slanju zahteva.');
      }
    } catch (err) {
      console.error(err);
      setCashOutMessage('Greška pri slanju zahteva.');
    } finally {
        ...
        <main className="grid gap-7 md:grid-cols-[1.8fr,1.2fr]">
          ...
          <section className="space-y-6">
            ...
            <div className="rounded-3xl border border-emerald-800/60 bg-gradient-to-r from-[#02110b] via-black to-[#02110b] p-6 shadow-lg shadow-emerald-500/20">
              ...
              <p className="mt-2 font-sans text-[11px] text-emerald-100/70">
                Vrednost tvog paketa.
              </p>
            </div>
            ...
            <div className="rounded-3xl border border-emerald-800/60 bg-black/80 p-6 shadow-xl shadow-emerald-500/20">
              ...
              <div className="mt-6 flex flex-wrap items-center gap-3">
                ...
                {hasPlan && (
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-full border border-emerald-600 px-5 py-2.5 text-[14px] font-sans font-semibold uppercase tracking-[0.16em] text-emerald-200 transition-all duration-200 hover:bg-emerald-500/10 hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {cashOutLoading ? 'Slanje...' : 'Cash out'}
                  </button>
                )}
              ...
              {cashOutMessage && (
                <p className={`mt-3 text-[12px] font-sans ${
                  cashOutMessage.includes('poslat') ? 'text-emerald-300' : 'text-red-400'
                }`}>
                  {cashOutMessage}
                </p>
              )}
