  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-b from-black via-emerald-950 to-black text-slate-50">
      ...
      <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <section className="mb-10 rounded-3xl border border-emerald-800/60 bg-gradient-to-r from-[#02110b] via-black to-[#02110b] p-6 sm:p-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {mostPopular.map((item) => (
              <div
                key={item}
                className="px-4 py-3 rounded-xl bg-black/60 border border-emerald-500/30 text-center font-sans text-[14px] text-emerald-100 hover:bg-emerald-500/10 hover:border-emerald-400 transition-all"
              >
                {item}
              </div>
            ))}
        ...
        {/* Forex Section */}
        <section className="mb-10 rounded-3xl border border-emerald-800/60 bg-black/80 p-6 sm:p-8">
          <h3 className="font-display text-[18px] sm:text-[22px] font-extrabold tracking-[0.08em] text-emerald-300 mb-3">
            {t('instruments.forex', lang)}
          </h3>
          <p className="font-sans text-[14px] text-emerald-100/90 mb-4">
            {t('instruments.forexDesc', lang)}
          </p>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-emerald-100/70 font-sans text-[13px]">
            {forexPairs.map((pair) => (
              <div key={pair} className="flex items-center">
                <span className="mr-2 text-emerald-400">•</span> {pair}
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-10 rounded-3xl border border-emerald-800/60 bg-black/80 p-6 sm:p-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-emerald-100/70 font-sans text-[13px]">
            {indices.map((index) => (
              <div key={index} className="flex items-center">
                <span className="mr-2 text-emerald-400">•</span> {index}
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-emerald-100/70 font-sans text-[13px]">
            {stocks.map((stock) => (
              <div key={stock} className="flex items-center">
                <span className="mr-2 text-emerald-400">•</span> {stock}
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-emerald-100/70 font-sans text-[13px]">
            {etfs.map((etf) => (
              <div key={etf} className="flex items-center">
                <span className="mr-2 text-emerald-400">•</span> {etf}
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-emerald-100/70 font-sans text-[13px]">
            {cryptocurrencies.map((crypto) => (
              <div key={crypto} className="flex items-center">
                <span className="mr-2 text-emerald-400">•</span> {crypto}
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-10 rounded-3xl border border-emerald-800/60 bg-black/80 p-6 sm:p-8">
          ...
          <ul className="space-y-2 text-emerald-100/70 font-sans text-[13px]">
            <li className="flex items-center">
              <span className="mr-2 text-emerald-400">•</span> Oil futures
            </li>
            <li className="flex items-center">
              <span className="mr-2 text-emerald-400">•</span> Stock index futures
            </li>
            <li className="flex items-center">
              <span className="mr-2 text-emerald-400">•</span> Commodity futures
            </li>
            <li className="flex items-center">
              <span className="mr-2 text-emerald-400">•</span> Currency futures
            </li>
          </ul>
        </section>

        {/* Other Markets Section */}
        <section className="mb-12 rounded-3xl border border-emerald-800/60 bg-black/80 p-6 sm:p-8">
          <h3 className="font-display text-[18px] sm:text-[22px] font-extrabold tracking-[0.08em] text-emerald-300 mb-3">
          ...
          <ul className="space-y-2 text-emerald-100/70 font-sans text-[13px]">
            {t('instruments.otherItems', lang).split(',').map((item, idx) => (
              <li key={idx} className="flex items-start">
                <span className="mr-2 text-emerald-400 mt-0.5">•</span>
                <span>{item.trim()}</span>
              </li>
            ))}
          </ul>
        ...
        {/* Contact Section */}
        <section className="text-center bg-gradient-to-r from-emerald-500/10 via-transparent to-emerald-500/10 rounded-3xl border border-emerald-500/30 p-8 mb-12">
